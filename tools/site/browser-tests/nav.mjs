...
// ---- blog filter: mouse, keyboard, touch; existing categories intact; no duplicate cards
{
  ...
  t('blog: all guides shown once each (' + all.length + ' cards, no duplicates)', all.length === 23 && new Set(all).size === 23, all.length + ' / ' + new Set(all).size);
  ...
  t('blog filter (keyboard): Enter on "All Guides" restores all 23', (await visibleCards(page)).length === 23);
  ...
  t('blog filter (keyboard): Space on "Waterproofing" filters, and no flooring-only guide leaks in', wp.length >= 1 && wp.length < 23 && wp.every((h) => cats.find(([x]) => x === h)[1].includes('waterproofing')));
...
}
{
  ...
  t('blog filter (touch): tapping All Guides shows twenty-three', (await visibleCards(page)).length === 23);
