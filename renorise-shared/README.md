...
**Status (2026-09-19): migration 0003 applied to production; forms Worker and dashboard deployed from branch `followup-sequence-v2` (not yet merged to `main`); Resend webhook configured and verified end to end (forged/stale/tampered requests refused; a real Resend delivery event was received and recorded); six real test emails sent to hello@renosrise.com and levi.gene.ous@gmail.com; the global sending switch is OFF; no customer is enrolled. Still to do: review the tests, then decide on merging and on turning the switch ON.**
